...
using eShopFlower.Utilities.Exceptions;
...
namespace eShopFlower.Application.Catalog.Products
{
	public class ProductService : IProductsService
	{
		...
		public ProductService(eShopFlowerDbContext context, IStorageService storageService)
		{
			_context = context;
		...
		{
			var product = await _context.Products.FindAsync(productId);
			if (product == null) throw new eShopFlowerException($"Cannot find a product: {productId}");
		...
		public async Task<PagedResult<ProductViewModel>> GetAllPaging(GetManageProductPadingRequest request)
		{
			//1.Select join
		...
		public async Task<ProductImageViewModel> GetImageById(int imageId)
		{
			...
			if (image == null)
				throw new eShopFlowerException($"Cannot find an image with id {imageId}");
		...
		public async Task<int> Update(ProductUpdateRequest request)
		{
			...
			if (product == null || productTranslations == null) throw new eShopFlowerException($"Cannot find a product with id: {request.Id}");
		...
		{
			var product = await _context.Products.FindAsync(productId);
			if (product == null) throw new eShopFlowerException($"Cannot find a product with id: {productId}");
		...
		public async Task<ApiResult<bool>> CategoryAssign(int id, CategoryAssignRequest request)
		{
			...
			{
				var productCategory = await _context.ProductInCategories.
